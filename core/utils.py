...
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, filename):
    """進行風格轉換和重構，然後保存結果圖像"""
    N, C, H, W = x_src.size()
    s_ref = nets.style_encoder(x_ref, y_ref)  # 獲取風格參考的風格編碼
    masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None  # 獲取特徵注意力圖
    x_fake = nets.generator(x_src, s_ref, masks=masks)  # 生成假圖像
    s_src = nets.style_encoder(x_src, y_src)  # 獲取源圖像的風格編碼
    masks = nets.fan.get_heatmap(x_fake) if args.w_hpf > 0 else None  # 獲取生成圖像的特徵注意力圖
    x_rec = nets.generator(x_fake, s_src, masks=masks)  # 重構圖像
    x_concat = [x_src, x_ref, x_fake, x_rec]  # 組合所有圖像進行保存
    x_concat = torch.cat(x_concat, dim=0)
    save_image(x_concat, N, filename)  # 保存圖像
    del x_concat
